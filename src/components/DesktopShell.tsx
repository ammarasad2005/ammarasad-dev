...
export function DesktopShell({ ideOpen, initialFile, onOpen, onMinimize, onRestart, reducedMotion, platform }: DesktopShellProps) {
  ...
  return (
    <div className={`desktop-shell platform-${platform} ${platform === 'macos' ? `mac-wallpaper-${macWallpaperTone}` : ''} ${focusMode ? 'focus-mode' : ''}`} onContextMenu={handleContextMenu} onPointerDown={(event) => { if (!(event.target as HTMLElement).closest('.desktop-context-menu, .mac-view-options')) setContextMenu(null) }}>
      ...
      <button className="desktop-person-widget" onClick={() => { setRequestedProjectId(null); openNative('projects') }}><AccountAvatar platform={platform} className="widget-avatar" size={90} /><span><small>CENTRALIZED WORKSPACE</small><strong>Open {platform === 'macos' ? 'Finder' : 'Explorer'}</strong><em>Projects · skills · resume · contact</em></span><ChevronStats /></button>
      ...
      <AnimatePresence>{startOpen && (platform === 'macos' ? <motion.section className="mac-launchpad" initial={{ opacity: 0, scale: 1.03 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.02 }}>
        ...
        <footer><AccountAvatar platform={platform} className="mini-avatar" size={54} /><div><strong>{greeting}, Ammar</strong><small>{focusMode ? `${focusIntent} zone · ${focusMinutes} min` : 'Everything you need, without the clutter.'}</small></div><button className={focusMode ? 'active' : ''} onClick={toggleFocusMode}><BrainCircuit />{focusMode ? 'End focus' : 'Start focus'}</button></footer>
        ...
        <footer><AccountAvatar platform={platform} className="mini-avatar" size={54} /><strong>Muhammad Ammar Asad</strong><span className="start-footer-status"><i />{focusMode ? 'Focused' : 'Available'}</span><button onClick={onRestart}>Restart</button></footer>
      ...
      <AnimatePresence>{socialPreview && <motion.aside className={`social-preview ${socialPreview}`} initial={{ opacity: 0, y: 12, scale: .97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8 }}>
        ...
        <div className="social-preview-cover"><AccountAvatar platform={platform} className="social-account-avatar" size={110} /></div>
