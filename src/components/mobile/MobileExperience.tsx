...
import { Apple, BatteryCharging, ChevronDown, ChevronUp, CirclePower, Cpu, ShieldCheck, Smartphone, Volume2, Wifi } from 'lucide-react'
import { MobilePortfolio } from '../MobilePortfolio'
import { AndroidShell } from './AndroidShell'
import { IOSShell } from './IOSShell'
import { AndroidMark } from './MobileMarks'
import { greetingFor, useClock } from './mobileSystem'
...
function MobileLoading({ os, reducedMotion }: { os: MobileOS; reducedMotion: boolean }) {
  ...
  return <main className="mobile-android-boot"><AndroidMark className="android-boot-mark" /><strong>AmmarOS</strong><em className="android-boot-line">verified boot · portfolio_a · built by Ammar</em><div className={`android-boot-dots ${reducedMotion ? 'still' : ''}`}><i /><i /><i /><i /></div><small>powered by <b>Android</b></small></main>
}
...
function MobileLockScreen({ os, now, onUnlock, onRestart, touchStartRef }: LockProps) {
  const [hour, minute] = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false }).split(':')
  const gestureProps = { onTouchStart: (event: React.TouchEvent<HTMLElement>) => { touchStartRef.current = event.touches[0].clientY }, onTouchEnd: (event: React.TouchEvent<HTMLElement>) => { if (touchStartRef.current !== null && touchStartRef.current - event.changedTouches[0].clientY > 45) onUnlock(); touchStartRef.current = null } }
  if (os === 'android') return <main className="mobile-lock-screen mobile-lock-android android-lock-native" {...gestureProps}><NextImage className="mobile-os-wallpaper" src="/mobile/android-wallpaper.webp" fill priority sizes="100vw" alt="" /><header className="mobile-lock-status"><span>AmmarPhone</span><div><Wifi /><BatteryCharging /><small>92</small></div></header><section className="android-lock-glance"><span>{now.toLocaleDateString([], { weekday: 'long', month: 'long', day: 'numeric' })}</span><small>Islamabad · {greetingFor(now)}, welcome to Ammar&rsquo;s workspace</small></section><section className="android-lock-clock"><strong>{hour}</strong><strong>{minute}</strong></section><section className="mobile-lock-notification android-lock-notification"><span><NextImage src="/ammar-avatar.png" width={64} height={64} alt="Ammar account" /></span><div><strong>Developer workspace</strong><p>4 shipped products and 12+ campus tools are ready.</p></div><small>now</small></section><button className="mobile-unlock-action" onClick={onUnlock}><i className="mobile-fingerprint" />Touch to unlock</button><button className="mobile-lock-restart" onClick={onRestart}><CirclePower /> Boot options</button></main>
  return <main className="mobile-lock-screen mobile-lock-ios" {...gestureProps}><NextImage className="mobile-os-wallpaper" src="/mobile/ios-wallpaper.webp" fill priority sizes="100vw" alt="" /><div className="ios-lock-island"><i /></div><header className="mobile-lock-status"><span>AmmarOS</span><div><Wifi /><BatteryCharging /><small>92</small></div></header><section className="mobile-lock-time"><span>{now.toLocaleDateString([], { weekday: 'long', month: 'long', day: 'numeric' })}</span><strong>{now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</strong></section><section className="mobile-lock-notification"><span><NextImage src="/ammar-avatar.png" width={64} height={64} alt="Ammar account" /></span><div><strong>Portfolio ready</strong><p>{greetingFor(now)} — Muhammad Ammar Asad’s mobile workspace is unlocked for you.</p></div><small>now</small></section><button className="mobile-unlock-action" onClick={onUnlock}>⌃<span>swipe up to open</span></button><button className="mobile-lock-restart" onClick={onRestart}><CirclePower /> Boot options</button></main>
}
