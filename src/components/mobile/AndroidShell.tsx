...
import { BatteryCharging, Bluetooth, Braces, Bug, CircleUserRound, FileText, Flashlight, FolderGit2, Github, GraduationCap, Hammer, Languages, Linkedin, Mail, Moon, Pause, Play, RotateCw, Search, Settings, ShieldCheck, Signal, SkipBack, SkipForward, Sparkles, Sun, Wifi, X } from 'lucide-react'
...
/** Material You "Modes" — the desktop Focus zone, expressed as an Android mode tile. */
const intentIcon: Record<SessionIntent, typeof Hammer> = { Build: Hammer, Learn: GraduationCap, Debug: Bug }
const devLanguages = ['TS', 'JS', 'PY']
...
export function AndroidShell({ onRestart }: AndroidShellProps) {
  ...
  const brightness = useSliderValue(78)
  ...
  function goBack() {
    if (activeApp) { if (appBackRef.current?.()) return; setActiveApp(null) }
    else if (drawerOpen || shadeOpen || recentsOpen) closeOverlays()
  ...
  return <main className={`mobile-os-root android-shell android-shell-native page-${page}`} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
    ...
    <button className="android-status-bar" onClick={() => setShadeOpen(true)} aria-label="Open Android notification shade"><span>{now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span><div>{focusMode && <em>{focus.intent}</em>}{doNotDisturb && <Moon />}<Signal />{internet && <Wifi />}<BatteryCharging /><small>{resilience}%</small></div></button>

    <div className="android-home android-home-native">
      <section className="android-at-glance"><div><span>{now.toLocaleDateString([], { weekday: 'long' })}</span><strong>{now.toLocaleDateString([], { month: 'long', day: 'numeric' })}</strong></div><small>Islamabad · {focusCopy ? `${focusCopy.headline} · ${focus.minutes}m` : `${rhythm.label} · ${rhythm.value}`}</small></section>
      <button className="android-search-pill" onClick={() => setDrawerOpen(true)}><Search /><span>Search Ammar&rsquo;s phone</span><i>G</i></button>
      {page === 0 ? <><button className="android-profile-widget" onClick={() => openApp('profile')}><span><NextImage src="/ammar-avatar.png" width={100} height={100} alt="Ammar account" /></span><div><small>{greetingFor(now).toUpperCase()}</small><strong>Muhammad Ammar</strong><p>Full-stack developer · open to internships</p></div></button><section className="android-project-widget" onClick={() => openApp('projects')}><header><span>Project activity</span><small>4 shipped</small></header><strong>{media.track.title}</strong><p>{media.track.subtitle} · tap to open the library.</p><div><i />Next.js <i />Supabase <i />TypeScript</div></section></> : <section className="android-social-dashboard"><header><span><NextImage src="/ammar-avatar.png" width={86} height={86} alt="Ammar" /></span><div><small>PROFESSIONAL</small><strong>Build. Share. Connect.</strong><p>Code and career, presented natively.</p></div></header><div><button onClick={() => openApp('github')}><Github /><span><strong>GitHub</strong><small>4 featured repositories</small></span></button><button onClick={() => openApp('linkedin')}><Linkedin /><span><strong>LinkedIn</strong><small>Open to work</small></span></button></div><footer><span>Portfolio completion</span><i><b style={{ width: '94%' }} /></i><strong>94%</strong></footer></section>}
    ...
    <nav className="android-dock android-dock-native" aria-label="Android favorites"><button onClick={() => openApp('profile')} aria-label="About Ammar"><CircleUserRound /></button><button onClick={() => openApp('projects')} aria-label="Projects"><FolderGit2 /></button><button onClick={() => openApp('contact')} aria-label="Contact"><Mail /></button><button onClick={() => setDrawerOpen(true)} aria-label="Open the app drawer"><span className="android-drawer-glyph"><i /><i /><i /><i /><i /><i /><i /><i /><i /></span></button></nav>
    ...
    {drawerOpen && <section className="android-app-drawer android-drawer-native"><header><label><Search /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search apps" /></label><button onClick={() => setDrawerOpen(false)} aria-label="Close the app drawer"><X /></button></header><span>ALL APPS</span><div>{visibleApps.map(({ id, label, icon: Icon, tone }) => <button key={id} onClick={() => openApp(id)}><span className={`mobile-app-icon tone-${tone}`}><Icon /></span><small>{label}</small></button>)}</div><p className="android-drawer-hint"><Sparkles /> {visibleApps.length === allApps.length ? 'Every app here is a real part of the portfolio.' : `${visibleApps.length} match${visibleApps.length === 1 ? '' : 'es'} for “${query}”.`}</p><footer><button onClick={onRestart}><Settings /> Boot &amp; system options</button></footer></section>}

    {shadeOpen && <section className="android-shade android-shade-native"><header><div><strong>{now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</strong><span>{now.toLocaleDateString([], { weekday: 'long', month: 'long', day: 'numeric' })}</span></div><div><button onClick={onRestart} aria-label="Boot and system options"><Settings /></button><button onClick={() => setShadeOpen(false)} aria-label="Close the notification shade"><X /></button></div></header>
      <section className="android-qs-primary"><button className={internet ? 'active' : ''} onClick={() => setInternet((value) => !value)}><Wifi /><span><strong>Internet</strong><small>{internet ? 'Builder Network' : 'Off'}</small></span></button><button className={bluetooth ? 'active' : ''} onClick={() => setBluetooth((value) => !value)}><Bluetooth /><span><strong>Bluetooth</strong><small>{bluetooth ? 'AmmarBook' : 'Off'}</small></span></button></section>
      <section className="android-qs-circles"><button className={focusMode ? 'active' : ''} onClick={() => focus.toggle('Build')}><Sparkles /><small>{focusMode ? focus.intent : 'Focus'}</small></button><button className={doNotDisturb ? 'active' : ''} onClick={() => setDoNotDisturb((value) => !value)}><Moon /><small>Do Not Disturb</small></button><button className={flashlight ? 'active' : ''} onClick={() => setFlashlight((value) => !value)}><Flashlight /><small>Flashlight</small></button><button className={rotation ? 'active' : ''} onClick={() => setRotation((value) => !value)}><RotateCw /><small>Auto-rotate</small></button></section>
      {focusMode && <><div className="android-focus-modes">{sessionIntents.map((intent) => { const Icon = intentIcon[intent]; return <button key={intent} className={focus.intent === intent ? 'active' : ''} onClick={() => focus.start(intent)}><Icon /><span>{intent}</span></button> })}</div><div className="android-focus-meter"><span>{focusCopy?.detail}</span><span>{focus.minutes}m · {doNotDisturb ? 'external noise shielded' : 'notifications allowed'}</span></div></>}
      <div className="android-brightness"><Sun /><i {...brightness.bind(false)} role="slider" aria-label="Screen brightness" aria-valuenow={brightness.value} aria-valuemin={0} aria-valuemax={100} tabIndex={0}><b style={{ width: `${brightness.value}%` }} /></i><button onClick={() => setResilience(100)} aria-label="Top up mental resilience"><ShieldCheck />{resilience}%</button></div>
      <section className="android-media-player"><span style={{ background: media.track.artwork }}><NextImage src="/ammar-avatar.png" width={64} height={64} alt="Ammar" /></span><div><small>{media.playing ? 'NOW BUILDING' : 'BUILD QUEUE'}</small><strong>{media.track.title}</strong><p>{media.track.subtitle}</p><i className="android-media-progress"><b style={{ width: `${Math.min(100, media.progress)}%` }} /></i></div><div className="android-media-controls"><button onClick={media.previous} aria-label="Previous build"><SkipBack /></button><button className="play" onClick={media.toggle} aria-label={media.playing ? 'Pause the build' : 'Resume the build'}>{media.playing ? <Pause /> : <Play />}</button><button onClick={media.next} aria-label="Next build"><SkipForward /></button></div></section>
      <div className="android-lang-strip"><Languages /><span>Dev language</span><div>{devLanguages.map((item) => <button key={item} className={language === item ? 'active' : ''} onClick={() => setLanguage(item)}>{item}</button>)}</div></div>
      <section className="android-notifications"><header><strong>Notifications</strong><button onClick={() => setNotificationsVisible(false)}>Clear all</button></header>{notificationsVisible ? <><button onClick={() => openApp('projects')}><FolderGit2 /><span><strong>Projects ready</strong><small>4 deployed builds are available to explore.</small></span><i>now</i></button><button onClick={() => openApp('github')}><Github /><span><strong>GitHub</strong><small>Repository activity and source links.</small></span><i>2m</i></button><button onClick={() => openApp('contact')}><Mail /><span><strong>Open to opportunities</strong><small>Full-stack internship availability is active.</small></span><i>today</i></button></> : <p>No notifications</p>}</section>
      <footer>Android 15 · Material You · AmmarPhone · {internet ? 'Builder Network' : 'offline'}</footer></section>}

    {recentsOpen && <section className="android-recents"><header><span>Recent apps</span><button onClick={() => setRecentsOpen(false)} aria-label="Close recent apps"><X /></button></header><div><button onClick={() => openApp('projects')}><span><FolderGit2 /> Projects</span><section><strong>Project Library</strong><p>FAST Utilities, WayFinder, GCR Fetch, DramaGhar</p></section></button><button onClick={() => openApp('github')}><span><Github /> GitHub</span><section><strong>@ammarasad2005</strong><p>Four featured repositories</p></section></button><button onClick={() => openApp('contact')}><span><Mail /> Contact</span><section><strong>New message</strong><p>Open to full-stack internships</p></section></button></div><footer><button onClick={() => setRecentsOpen(false)}>Screenshot</button><button onClick={() => { setRecentsOpen(false); setActiveApp(null) }}>Select</button></footer></section>}

    {activeApp && <MobileAppContent key={activeApp} app={activeApp} os="android" now={now} resilience={resilience} focusLabel={focusLabel} online={internet} onBack={goBack} onOpenApp={openApp} backRef={appBackRef} />}
